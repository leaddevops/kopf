...
from typing import Collection, Iterable, Iterator, List, Mapping, \
                   MutableMapping, NewType, Optional, Set, Union

# A namespace specification with globs and minimal syntax; see `match_namespace()`.
NamespaceSpec = NewType('NamespaceSpec', str)
...
def select_specific_namespaces(specs: Iterable[NamespaceSpec]) -> Collection[NamespaceName]:
    """
    ...
    return {
        NamespaceName(spec)
        for spec in specs
        if not('!' in spec or '*' in spec or '?' in spec or ',' in spec)
    }


def match_namespace(name: NamespaceName, spec: NamespaceSpec) -> bool:
    """
    ...
    * if the the first glob is exclusive, then a preceding catch-all is implied.
    ...
    globs = [glob.strip() for glob in spec.split(',')]
...
class ResourceMarkers(enum.Enum):
    """
...
# An explicit catch-all marker for positional arguments of the resource specs.
EVERYTHING = ResourceMarkers.EVERYTHING
...
class ResourceSpec:
    ...
    resources with specific names (:class:`ResourceRef`). The handlers are
    ...
    arg1: dataclasses.InitVar[Union[str, ResourceMarkers, None]] = None
    arg2: dataclasses.InitVar[Union[str, ResourceMarkers, None]] = None
    arg3: dataclasses.InitVar[Union[str, ResourceMarkers, None]] = None
    ...
    any_name: Optional[Union[str, ResourceMarkers]] = None

    def __post_init__(
            self,
            arg1: Union[str, ResourceMarkers, None],
            arg2: Union[str, ResourceMarkers, None],
            arg3: Union[str, ResourceMarkers, None],
        ...
        if argN is not None:
            ...
            object.__setattr__(self, 'group', arg1)
            object.__setattr__(self, 'any_name', arg2)
        elif arg1 is not None and isinstance(arg1, ResourceMarkers):
    ...
    def is_specific(self) -> bool:
        return (self.kind is not None or
                ...
                (self.any_name is not None and not isinstance(self.any_name, ResourceMarkers)))
    ...
    def check(
        ...
        return (
            ...
            (self.any_name is None or
             self.any_name is ResourceMarkers.EVERYTHING or
...
CRDS = ResourceSpec('apiextensions.k8s.io', 'customresourcedefinitions')
EVENTS = ResourceSpec('v1', 'events')
NAMESPACES = ResourceSpec('v1', 'namespaces')
CLUSTER_PEERINGS = ResourceSpec('zalando.org/v1', 'clusterkopfpeerings')
NAMESPACED_PEERINGS = ResourceSpec('zalando.org/v1', 'kopfpeerings')


class Backbone(Mapping[ResourceSpec, ResourceRef]):
    """
    ...
    def __init__(self) -> None:
        super().__init__()
        self._items: MutableMapping[ResourceSpec, ResourceRef] = {}
    ...
    def __getitem__(self, item: ResourceSpec) -> ResourceRef:
...
class Insights:
    ...
    """
    namespaces: Set[NamespaceRef] = dataclasses.field(default_factory=set)
    resources: Set[ResourceRef] = dataclasses.field(default_factory=set)
